...
void KalmanFilter::predictionStep(double dt)
{
    if (!isInitialised() && INIT_ON_FIRST_PREDICTION)
    {
        ...
        // ENTER YOUR CODE HERE
            ...
            cov(0,1) = INIT_POS_STD*INIT_POS_STD;
    ...
    if (isInitialised())
    {
        ...
        // Defne the Q Matrix for the process noise
        MatrixXd Q = Matrix2d::Zero();
        Q(0,0) = ACCEL_STD*ACCEL_STD;
        Q(1,1) = ACCEL_STD*ACCEL_STD;

        // Define the L Matrix for the process noise
...
void KalmanFilter::handleGPSMeasurement(GPSMeasurement meas)
{
    if(isInitialised())
    {
        ...
        // Set your H and R matrices. Set your measurement vector z
        ...
        MatrixXd S = H * cov * H.transpose() + R; // Innovation (Error) Covariance
        MatrixXd K = cov*H.transpose()*S.inverse(); // Optimal Kalman Gain

        state = state + K*y; // Updated State Estimate
        cov = (MatrixXd::Identity(4,4) - K*H) * cov; // Updated Estimate Covariance
    ...
    else
    {
        ...
        // ENTER YOUR CODE HERE
            ...
            // Set the initial state based on the first GPS measurement
            state(0) = meas.x;
            state(1) = meas.y;
